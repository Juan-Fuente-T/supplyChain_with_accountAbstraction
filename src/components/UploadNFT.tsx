...
// export async function UploadNFT(appSigner: Signer, address: string) {    // const [transmisionData, updateTransmisionData] = useState<{ Fecha: string; Participante: string; Clase: string; }[]>([]);
...
// export async function UploadNFT(appSigner: ethers.Signer, address: string, contract: Contract, provider: Provider): Promise<boolean> {
export async function UploadNFT(appSigner: ethers.Signer, address: string, contract: Contract, productId: number | undefined): Promise<boolean> {
    // Elimina todos los listeners previos para evitar duplicados
    contract.removeAllListeners('TransferOwnership');
    console.log("Signer en UploadNFT:", appSigner);
    ...
    const tokenContract = new Contract(TOKEN_CONTRACT_ADDRESS, supplyChainTokenABI, appSigner);

    ////////////////////BORRAR; ES PARA PRUEBAS LOCALES CON ANVIL SOLO///////////////
    // const tokenContract = new Contract("0x5FbDB2315678afecb367f032d93F642f64180aa3", supplyChainTokenABI, appSigner);
    // // const tokenContract = new Contract(TOKEN_CONTRACT_ADDRESS, supplyChainTokenABI, appSigner);
    ...
    // Obtén el appSigner del provider
    // const _appSigner = await _provider.getSigner(_address);

    // Firma el mensaje
    const _message = "Hola, Trazable DLT pagará el gas por ti";
    // const signature = await _appSigner.signMessage(_message);
    // const signature = await localWallet.signMessage(_message);
    const signature = await appSigner.signMessage(_message);
    ...
    return new Promise(async (resolve, reject) => {
        console.log("Signer en Promise:", appSigner);
        try {
            ...
            //         console.log("UP_productId", productId, _productId);
            ...
            isProcessing = true;
            try {
                console.log("tokenContract en UploadNFT", tokenContract);
                console.log("productId",productId);
                ...
                if (supplierType === "Consumer") {
                    ...
                    if (productId !== undefined) {
                        ...
                        if (metadataCid !== undefined) {
                            ...
                            // const success = true;
                            console.log("Signer justo antes de emitNFT", appSigner);
                            const success = await emitNFT(tokenContract, ethers.toBigInt(productId), metadataCid, address, filteredIds, appSigner);
                            if (success) {
    ...
    async function emitNFT(
        ...
        console.log("DATA", tokenContract, productId, metadataCid, address);
        if (!tokenContract || !metadataCid || !address || !productId) return false;
        try {
            console.log("Signer en MINT:", appSigner);
            console.log("Address conectada que usa la Dapp: ", address);
            const message = "Hola, Trazable DLT pagará el gas por ti";
            const hash = hashMessage(message);
          
          
            // let signature = await signMessage(message, appSigner);
            console.log("Message signed");
            // const hash = hashMessage(message);
            const signature = await signMessage(message, appSigner);
            console.log("signature", signature);
            try {
                // Estimar el gas
                const estimatedGas = await tokenContract.mint.estimateGas
                    (hash,
                        signature,
                        address,
                        productId,
            ...
            console.log("Is Hash a string?", typeof hash === 'string');
            const tx = await tokenContract.mint(hash, signature, address, "12", 1, metadataCid);
            await tx.wait();
            console.log('NFT emitted successfully');
    ...
    //         //After adding your Hardhat network to your metamask, this code will get providers and appSigners
    //         // const provider = new ethers.BrowserProvider(window.ethereum, "any"); // "any" permite trabajar con diferentes redes            const appSigner = provider.getSigner();
    //         disableButton();
